...
//std::string toUpper(std::string stringToChange)
//{
//	for (auto& c : stringToChange)
//	{
//		c = toupper(c);
//	}
...
void bubbleSort(std::vector<std::string>& A)
{
	...
	do
	{
		...
		for (int i = 1; i <= n - 1; i++)
		{
			...
			int strComp = toUpper(A[i - 1]).compare(toUpper(A[i]));
			if (strComp > 0)//A[i - 1] > A[i])//case-sensitive
			{
				//swap(A, i - 1, i)
				auto temp = A[i - 1];
				A[i - 1] = A[i];
